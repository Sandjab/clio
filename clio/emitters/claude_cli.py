...
from clio.emitters._shared_utils import _render_type_short
...
class ClaudeCLIEmitter(BaseEmitter):
    ...
    @staticmethod
    def _emit_readme(graph: FlowGraph) -> str | None:
        """Return a README.md string when the FLOW declares TAKES or GIVES.
        Returns None when neither is declared (v0.15 flows: no README emitted)."""
        flow = graph.flow
        if flow is None or (not flow.takes and not flow.gives):
            return None
        lines: list[str] = [
            f"# {flow.name}",
            "",
            "Generated by `clio compile --target claude-cli`.",
            "",
            "## FLOW signature",
            "",
        ]
        if flow.takes:
            lines.append("Declared external inputs (pass via `state.json`):")
            lines.append("")
            for f in flow.takes:
                lines.append(f"- `{f.name}: {_render_type_short(f.type)}`")
            lines.append("")
        if flow.gives:
            lines.append("Declared outputs (final `state.json` will contain at least):")
            lines.append("")
            for f in flow.gives:
                lines.append(f"- `{f.name}: {_render_type_short(f.type)}`")
            lines.append("")
