...
- **`examples/parallel_review.clio`** — a fan-out code review whose centre of gravity is `FOR EACH … PARALLEL`: changed files are reviewed concurrently, the drafts are triaged concurrently, and only the final summary joins the results. It compiles to `claude-workflow` (two `parallel()` fan-outs), `python`, `go`, `swift` and `mcp-server`; `claude-skill` serialises it with a warning, and `claude-cli` / `langgraph` reject it.
