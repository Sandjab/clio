...
- **`target: claude-workflow` — eighth compilation target.** Emits `<flow-name>.workflow.js` (the script), a `README.md` naming the exact stubs the author must fill in and repeating only the degradations *this* flow triggered, and a `.clio/` sidecar (the same writer `claude-skill` uses since v0.19), so `clio import` round-trips the source verbatim, hash-drift detection included. Mapping: a `judgment` step → `await agent(prompt, { label, phase, schema })`, with the step's `GIVES` contract **inlined** into a self-contained schema literal (no `$ref` — the sandbox cannot resolve one at run time) and the host forcing the subagent through it; an `exact` step → a **pure-JS stub** that throws until filled in; `FOR EACH … PARALLEL AS c` → `parallel()` over concurrent subagents, each item's outcome captured by the emitted thunk (`{ok, value}` / `{ok, error}`) so that a **failed item fails the flow** — the semantics `python` / `go` / `swift` already have, the step's own `ON_FAIL` chain having already run inside its function — while a **legitimate `null`** (an `Optional<T>` `GIVES`, valid against the schema since v0.21) survives in the collector, which stays index-aligned with the collection; sequential `FOR EACH` → `for…of`; `IF` / `MATCH` / `WHILE … MAX N` → native `if` / `switch` / bounded `while`; a sub-flow call → an **inlined** local `async function` (sidestepping the host's one-level nesting cap); `ON_FAIL` / `RESCUE` / `RESUME` → `try` / `catch` + retry loop / fallback step / abort. `agent()` returns **null** on terminal failure rather than throwing, so every emitted step wrapper converts that null into a thrown error — without it `ON_FAIL` and `RESCUE` would be dead code that passes every text assertion and never fires once. Model ids map to the tier enum (`claude-opus-*` → `'opus'`, `-sonnet-` → `'sonnet'`, `-haiku-` → `'haiku'`); when the source declares no model, `model` is omitted and the subagent inherits the session's. Eight emitter modules under `clio/emitters/`: `workflow.py`, `_workflow_helpers.py`, `_workflow_flow_renderer.py`, `_workflow_step_renderers.py`, `_workflow_expressions.py`, `_workflow_loops.py`, `_workflow_subflows.py`, `_workflow_errors.py`.

  **Refused at compile time**, with stable `E_WF_NNN` codes: no `FLOW` in the source (001); `invoke.api.openai / bedrock / vertex` — an `agent()` cannot call a non-Anthropic provider (002); `impl.mode: shell | rest | sql | mcp_tool` — the sandbox has no process, no network, no filesystem (003); an explicit `LANG:` other than `node` / `auto` (004); a `CONTRACT` reference **cycle**, which cannot be inlined (005); a source declaring **several `FLOW`s** with no `--flow` selection, since this target emits exactly one script and compiling the first would silently drop the others (006); a **recursive `FLOW`**, which inlining would turn into a stack overflow at run time (007).

  **Degraded, with a compile-time warning**: `CACHE:` is **ignored** (`W_WF_001` — no filesystem, no clock; a cache miss is slower, never wrong); `ON_FAIL` retries run **without backoff** (`W_WF_002` — `Date.now()` throws in the sandbox, so they fire back-to-back); `CONTRACT … ASSERT` is **not enforced** (`W_WF_003` — the host validates the JSON Schema's types, ranges and enums, but not the `ASSERT` predicate; use `--target python` if it is load-bearing).

- **`examples/parallel_review.clio`** — a fan-out code review whose centre of gravity is `FOR EACH … PARALLEL`: an exact step cuts the diff at each `diff --git` boundary, every file's hunks are then reviewed concurrently, every draft is triaged concurrently, and only the final summary joins the results. Two fan-outs rather than one two-step body, because a `PARALLEL` body is **exactly one** call (`LANGUAGE_SPEC.md`, *PARALLEL FOR EACH bodies*) — and the reviewer is handed the file's **diff**, not its path: a step given only `src/auth.py` has nothing to read, and a subagent will correctly refuse to review a file it cannot find. It compiles to `claude-workflow` (two `parallel()` fan-outs), `python`, `go`, `swift` and `mcp-server`; `claude-skill` serialises it with a warning, and `claude-cli` / `langgraph` reject it.

- **`node --check` gate in the test suite** — every compiled fixture is syntax-checked as an ES module, the mechanical gate `go build` / `swift build` give the native targets (skipped cleanly when `node` is absent).

### Changed

- **Target tables and counts updated from seven to eight** across `README.md`, `CLAUDE.md`, `docs/COMPILATION_TARGETS.md`, `docs/LANGUAGE_SPEC.md` and `docs/manual/04-targets.md` — including the cross-target feature matrix, which gains a `claude-workflow` column marking each refusal and degradation.

## [0.24.0] — 2026-06-07

Minor release adding **`target: swift`**, the seventh compilation target — a zero-dependency SwiftPM emitter (Swift 6, macOS + Linux) at MVP scope: exact + judgment + `CACHE` (byte-identical keys with the python/go targets) + `CONTRACT` + `ON_FAIL` + `IF/ELSE`/`MATCH`/`WHILE` + `FOR EACH` sequential & parallel (`withThrowingTaskGroup`). REST/shell (Phase 4), sub-flow composition + `RESCUE` (Phase 5), and FOR EACH over a loop variable are refused at compile time. Also ships the `/skill2clio` in-session skill→`.clio` import path (no API key) and a parser fix for `\"`/`\\` string-literal escapes.

### Added

- **`target: swift` — seventh compilation target.** Emits a **two-target** Swift package (a library `ClioFlow` plus an executable named after the flow) that builds with `swift build` on macOS and Linux. `Sources/<flow_name>/Main.swift` is the `@main` entry that parses `--kwargs` and calls `Flow.run(kwargs:)`; everything else lives under the library target `Sources/ClioFlow/` — `Flow.swift` (`public enum Flow { @MainActor public static func run(kwargs: [String: Any]) async throws -> [String: Any] }`), `Contracts.swift`, per-step files under `Steps/`, and a `Runtime/` directory. **Zero external SPM dependencies** — all runtime code is vendored pure Swift, emitted conditionally: `Runtime/Anthropic.swift` (URLSession Anthropic client with `#if canImport(FoundationNetworking)` for Linux; only when a judgment step is present), `Runtime/Validate.swift` (hand-rolled JSON Schema + `x-clio-assert` walker matching python/go semantics; only with a CONTRACT), `Runtime/Cache.swift` + `Runtime/SHA256.swift` (SHA256-keyed on-disk cache with keys **byte-identical** to the `python` and `go` targets — `.cache/` directories swap across targets; only when a step uses `CACHE`). MVP scope: `CONTRACT` lowers to `Codable, Sendable` structs with `static let jsonSchema` + `validate()`; `exact` steps (LANG `swift` or `auto`) emit a typed `func step_<name>` stub; `judgment` steps emit the URLSession Anthropic call; `IF/ELSE`, `MATCH/CASE` (with synthesized exhaustive `default`), `WHILE … MAX N` (bounded), and `FOR EACH` (sequential + `PARALLEL AS <collector>` via `withThrowingTaskGroup`, concurrency cap 10); `CACHE: ttl(...)` with byte-identical keys; `ON_FAIL` chain — `retry(N)` (exponential backoff via `Task.sleep`), `fallback(step)`, and `abort(msg)` are emitted, while `escalate` is a **no-op** (single model per emission, same as `target: go`). Five emitter modules: `clio/emitters/swift.py`, `_swift_helpers.py`, `_swift_flow_renderer.py`, `_swift_step_renderers.py`, `_swift_runtime_templates.py`. Refused at compile time with stable `E_SWIFT_NNN` codes: non-swift LANG on exact (001), `invoke.mode: cli` (002), bedrock/vertex (003), openai (005), sql (009), mcp_tool (010), TEST blocks (012), rest form/file/multipart body (013). Code `E_SWIFT_006` (multi-GIVES sub-flow as a PARALLEL body) is **reserved/dormant** — it cannot fire today because a multi-GIVES sub-flow requires more than one FLOW, which is already refused earlier (FLOW composition, Phase 5); it becomes reachable only once composition ships. Deferred: `impl.mode: rest` json/raw and `impl.mode: shell` (Phase 4); FLOW composition and `RESCUE`/`RESUME` (Phase 5); FOR EACH over a loop variable (planned later). **Known limitation**: a `FOR EACH PARALLEL AS <collector>` result is terminal-only — `clio compile` emits without error, but typed downstream consumption of the collector does not compile under `swift build`; use `--target python` or `--target go`.
...
- **Parser: string literals now support `\"` and `\\` escapes** (`clio/parser/lexer.py`). An inline `impl` shell `cmd:` emitting JSON (e.g. `cmd: "echo '{\"ok\": true}'"`) is now expressible; before, the unescaped `"` terminated the string early and tripped a misleading `unexpected character '\'`. Any other `\x` stays a literal backslash (no regression). Closes #88.
...
- **New `E_GO_013` refusal** — a rest step with a `form`/`file`/`multipart` body on `target: go` is rejected at compile time. The renderer only builds a body reader for json/raw bodies; other shapes previously fell through to a `nil` request body (a silent drop — the body was never sent, with no refusal or warning). The refusal is future-proof: any body that is neither `JsonBodyIR` nor `RawBodyIR` is caught.

### Known limitations (deferred to v0.24)

- A single-GIVES sub-flow used as a `FOR EACH PARALLEL` body is **terminal-only** on `target: go`: the collector is produced, but typed downstream consumption (`aggregate(xs=results)`, `FOR EACH x IN results`) fails the `go build`.
- `form`/`file`/`multipart` rest bodies are unsupported (E_GO_013) — use `--target python`.
