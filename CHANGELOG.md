...
- **`detect_skill_language` recognises uppercase French diacritics** (closes issue #40) — `fr_markers = set("éèàçôîêûïü")` was lowercase-only. A description opening with a capital diacritic (`Évaluer le risque...` — natural sentence-start French) was silently classified as EN. Fixed by case-folding the joined samples via `text.lower()` before scanning. The `test_cache_block_uses_fr_label_when_flow_is_french` test source now opens with `Évaluer` to actively exercise this path; new dedicated unit test `test_detect_skill_language_handles_uppercase_french_diacritics` covers the heuristic directly.
