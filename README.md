...
[![Version](https://img.shields.io/badge/Version-v0.24.0-green.svg)](https://github.com/Sandjab/clio/releases/tag/v0.24.0)
...
| `claude-workflow` | Claude Code Workflow script (a single JS module: `export const meta` + `agent()` / `parallel()` / `phase()`); host-orchestrated, no API key — and the only target where `FOR EACH … PARALLEL` runs concurrently, one subagent per item |
...
**v0.25.0 (unreleased)**: the **`claude-workflow` target** — an eighth compilation target emitting a **Claude Code Workflow script** (one JS module: `export const meta` + `agent()` / `parallel()` / `phase()`) that orchestrates **subagents**. Host-orchestrated like `claude-cli` and `claude-skill` — the Claude Code session is the runtime, so **no API key** and nothing to install — and the **only target where `FOR EACH … PARALLEL` is really parallel** (`claude-skill` serialises it with a warning, `claude-cli` rejects it). Judgment steps become `agent()` calls with the `GIVES` contract inlined into a self-contained schema; exact steps become pure-JS stubs; sub-flows are inlined; `IF`/`MATCH`/`WHILE` and `ON_FAIL`/`RESCUE`/`RESUME` all lower to native JS. The sandbox has no process, network, filesystem or clock, so `impl.mode: shell / rest / sql / mcp_tool` is refused (`E_WF_003`), and `CACHE` (`W_WF_001`), retry **backoff** (`W_WF_002`) and `CONTRACT … ASSERT` (`W_WF_003`) are degraded with a compile-time warning.

**v0.24.0**: the **`swift` target** — a **zero-dependency SwiftPM package** (Swift 6, macOS + Linux) at MVP scope: judgment (URLSession Anthropic client), `CACHE` (keys byte-identical with the python/go targets), `CONTRACT`, `ON_FAIL`, `IF`/`MATCH`/`WHILE`, and sequential & parallel `FOR EACH` (`withThrowingTaskGroup`). REST/shell (Phase 4), sub-flow composition + `RESCUE` (Phase 5), and FOR EACH over a loop variable are refused at compile time. On top of the **`go` target** (v0.23: REST/shell/FLOW composition), **`clio import`**, **`Dict<K, V>`** / **`Optional<T>`**, and extended primitive constraints from prior releases — **eight compilation targets** total. **1583 unit tests + 1 xfail + gated e2e (incl. 5 opt-in `e2e_llm`).**
