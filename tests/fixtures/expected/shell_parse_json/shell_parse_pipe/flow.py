...
def _persist_state(step_idx: int, state: dict) -> None:
    ...
    payload = {"version": 1, "flow": "shell_parse_pipe", "step_index": step_idx, "state": state}
    tmp = path + ".tmp"
...
def run(*, start_at: int = 0, **initial: object) -> dict:
    if start_at > 0:
        ...
        with open(path) as f:
            payload = json.load(f)
        if payload.get("flow") != "shell_parse_pipe":
            print(f'[clio] state.json flow mismatch: expected "shell_parse_pipe", got {payload.get("flow")!r}', file=sys.stderr)
            raise SystemExit(2)
    ...
    _log.set_flow("shell_parse_pipe")
    _log.emit("flow_start", resumed_from=start_at if start_at > 0 else 0)
    _success = False
    _t0 = time.monotonic()
    try:
        if start_at < 1:
            state['corpus'] = load_corpus_mod.load_corpus(file='data.json')
