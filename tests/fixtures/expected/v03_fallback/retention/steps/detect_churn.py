...
def detect_churn(*, customers: list[dict]) -> list[contracts.CustomerRisk]:
    ...
    prompt = prompt.replace('${customers}', json.dumps(customers, default=lambda o: o.model_dump() if hasattr(o, 'model_dump') else str(o)))
    ...
    if response is None:
        from . import detect_churn_naive as _detect_churn_naive_mod
        fb_response = _detect_churn_naive_mod.detect_churn_naive(customers=customers)
        response = (lambda raw: [contracts.CustomerRisk.model_validate(item) for item in raw])(fb_response if not isinstance(fb_response, str) else json.loads(fb_response))
        fallback_used = True
    ...
    _log.emit("step_end", step='detect_churn', mode="judgment",
              duration_ms=int((time.monotonic() - _t0) * 1000),
              cache_hit=False, model=_MODELS[model_idx],
              fallback_used=fallback_used, success=True, **_last_usage)
    return response
