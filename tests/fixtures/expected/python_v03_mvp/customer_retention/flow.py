...
def _persist_state(step_idx: int, state: dict) -> None:
    ...
    payload = {"version": 1, "flow": "customer_retention", "step_index": step_idx, "state": state}
    tmp = path + ".tmp"
...
def run(*, start_at: int = 0, **initial: object) -> dict:
    if start_at > 0:
        ...
        with open(path) as f:
            payload = json.load(f)
        if payload.get("flow") != "customer_retention":
            print(f'[clio] state.json flow mismatch: expected "customer_retention", got {payload.get("flow")!r}', file=sys.stderr)
            raise SystemExit(2)
    ...
    _log.set_flow("customer_retention")
