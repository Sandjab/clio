...
When a STEP / FLOW declares TAKES or GIVES fields whose names collide with
Python keywords (`from`, `class`, ...), every Python-emitting target must
still produce syntactically valid Python. The emit-side fix is to sanitize
the identifier (via `_to_field_name`) whenever it lands in a Python
identifier position (kwarg LHS, local variable LHS, function parameter),
