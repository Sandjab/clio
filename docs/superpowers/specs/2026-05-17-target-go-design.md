...
| `FOR EACH PARALLEL item IN coll` | `g, ctx := errgroup.WithContext(ctx); g.SetLimit(10); for _, item := range stateColl { g.Go(func() error { ... use item ... }) }; if err := g.Wait(); err != nil { return ... }` | Cap = 10, identical to python target's `ThreadPoolExecutor(max_workers=10)`. Go 1.22+ scopes the loop variable per iteration, so no `item := item` capture is needed. |
...
| `impl.mode sql` | `clio_runtime/sql.Query(ctx, dbName, query, bindings)` returning `[]map[string]any` for SELECT, `int64` rowcount for write ops | One long-lived `*sql.DB` per database (already a thread-safe connection pool — no manual `sync.Mutex` needed, in contrast to the python target where `threading.Lock` serialises access because sqlite3 connections are not concurrency-safe by default). Lazy driver registration. |
| `impl.mode mcp_tool` | `clio_runtime/mcpclient.CallTool(ctx, server, toolName, args)` returning `map[string]any` | Lazy per-server clients (stdio subprocess or HTTP session). Lifetime is bound to the orchestrating `context.Context`: on context cancellation each client's `Close()` runs via a goroutine guarded by `<-ctx.Done()`; a single top-level `Shutdown()` exposed on the package also closes them all. No `runtime.SetFinalizer` (non-deterministic, not guaranteed to run promptly). |
