...
def refine_summary(*, article: str, draft: str, review: contracts.SummaryJudgment) -> str:
    _t0 = time.monotonic()
    _log.emit("step_start", step='refine_summary', mode="judgment")
    _last_usage: dict = {}
    ...
    primary_key = _cache.cache_key('refine_summary', _MODELS[0], prompt, _INLINED_SCHEMA)
    hit = _cache.cache_lookup(cache_dir, 'refine_summary', primary_key, 604800)
    if hit is not None:
        try:
            _ret = (lambda raw: raw)(json.loads(hit))
            _log.emit("step_end", step='refine_summary', mode="judgment",
    ...
    if response is None and model_idx < len(_MODELS) - 1:
        model_idx += 1
        esc_key = _cache.cache_key('refine_summary', _MODELS[model_idx], prompt, _INLINED_SCHEMA)
        esc_hit = _cache.cache_lookup(cache_dir, 'refine_summary', esc_key, 604800)
        if esc_hit is not None:
            try:
                _ret = (lambda raw: raw)(json.loads(esc_hit))
                _log.emit("step_end", step='refine_summary', mode="judgment",
                          duration_ms=int((time.monotonic() - _t0) * 1000),
        ...
        response = _attempt(_MODELS[model_idx], prompt)
        if response is not None:
            _cache.cache_store(cache_dir, 'refine_summary', esc_key, _MODELS[model_idx], _serialize(response))

    if response is None:
        print('[clio] step refine_summary: refine_summary failed', file=sys.stderr)
        _log.emit("step_end", step='refine_summary', mode="judgment",
                  duration_ms=int((time.monotonic() - _t0) * 1000),
    ...
    if model_idx == 0 and response is not None:
        _cache.cache_store(cache_dir, 'refine_summary', primary_key, _MODELS[0], _serialize(response))
